...
# NOTE: It does not change the probability of conversion upon interaction with a friend
...
class Environment:
    ...
    def step(self) -> None:
        ...
        2. Interacts with social circle, regardless of proximity
        3. Interacts with any agents in adjacent cells
        """
        ...
        # Handle interactions
        for agent in self.agents:
            # First handle interactions within social circle 
            for friend in agent.friends:
                agent.interact(friend)
            # Then handle proximity interactions
            x, y = agent.position
...
class Agent:
    def __init__(self, position: Tuple[int, int], opinion: int = 0):
        ...
        self.friends: list['Agent'] = [] # social circle
...
if __name__ == "__main__":
    ...
    # After creating all agents, assign up to 5 random friends:
    for agent in all_agents:
        agent.friends = random.sample([a for a in all_agents if a != agent], k=random.randint(1, 5))  
